...
for index, row in media.iterrows():
    ...
    if 'thumbnail' in metadata and 'thumbnail_path' in metadata['thumbnail'] and os.path.isfile(metadata['thumbnail']['thumbnail_path']):
        if int(metadata['thumbnail']['thumbnail_height']) == row['thumbnail_height'] and int(metadata['thumbnail']['thumbnail_width']) == row['thumbnail_width']:
            # Focus only on unrotated thumbnails
            continue
